...
public class QueryScreenItemHandler extends EntireJTagHandler
{
    private EJPluginQueryScreenItemProperties _itemProperties;
    private static final String               ELEMENT_QUERY_SCREEN_RENDERER_ITEM_PROPERTIES = "queryScreenRendererItemProperties";
    ...
    public QueryScreenItemHandler(EJPluginItemGroupProperties itemGroupProperties)
    {
        _itemGroupProperties = itemGroupProperties;
    }
    
    public EJPluginQueryScreenItemProperties getItemProperties()
    {
    ...
    public void startLocalElement(String name, Attributes attributes) throws SAXException
    {
        if (name.equals(ELEMENT_ITEM))
        {
            ...
            {
                _itemProperties = new EJPluginQueryScreenItemProperties(_itemGroupProperties, false, false);
                _itemProperties.setReferencedItemName(referencedItemName);
            }
        }
        else if (name.equals(ELEMENT_QUERY_SCREEN_RENDERER_ITEM_PROPERTIES))
        {
            // Now I am starting the selection of the renderer item
            // properties
            setDelegate(new FrameworkExtensionPropertiesHandler(_itemProperties.getBlockProperties().getFormProperties(),
                    _itemGroupProperties.getBlockProperties(), ELEMENT_QUERY_SCREEN_RENDERER_ITEM_PROPERTIES));
        }
    }
    ...
    {
        if (name.equals(ELEMENT_QUERY_SCREEN_RENDERER_ITEM_PROPERTIES))
        {
            if (((FrameworkExtensionPropertiesHandler) currentDelegate).getMainPropertiesGroup() != null)
            {
                if (_itemProperties.getBlockProperties().getBlockRendererDefinition() != null
                        && _itemProperties.getBlockProperties().getBlockRendererDefinition().getQueryScreenRendererDefinition() != null)
                {
                    _itemProperties.setQueryScreenRendererRequiredProperties(((FrameworkExtensionPropertiesHandler) currentDelegate)
                            .getMainPropertiesGroup(_itemProperties.getBlockProperties().getBlockRendererDefinition().getQueryScreenRendererDefinition()
                                    .getQueryScreenPropertyDefinitionGroup()));
                }
                else
                {
                    _itemProperties.setQueryScreenRendererRequiredProperties(((FrameworkExtensionPropertiesHandler) currentDelegate).getMainPropertiesGroup());
                }
            }
        }
    }
