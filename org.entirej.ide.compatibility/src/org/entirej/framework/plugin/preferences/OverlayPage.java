...
public abstract class OverlayPage extends PropertyPage
{
    ...
    public void createControl(Composite parent)
    {
        ...
        if (isPropertyPage()) setControlsEnabled();
    }
    ...
    private void setControlsEnabled(Composite root, boolean enabled)
    {
        ...
        for (int i = 0; i < children.length; i++)
        {
            ...
            if (!(child instanceof CTabFolder) && !(child instanceof TabFolder) && !(child instanceof PageBook)) child.setEnabled(enabled);
            if (child instanceof Composite) setControlsEnabled((Composite) child, enabled);
        }
    }
